Shader "Main Cel" {
    ...
    SubShader {
        ...
        float4 LightingStepped(SurfaceOutput s, float3 lightDir, half3 viewDir, float shadowAttenuation){
            ...
            float steppedDiff = round(diff * _RampLevels) / _RampLevels;
            steppedDiff = max(0, steppedDiff); // Prevent negative values

            // float3 diffuse = _LightColor0.rgb * steppedDiff * s.Albedo;
 
            float towardsLightChange = fwidth(diff);
            float lightIntensity = smoothstep(0, towardsLightChange, diff);
            float3 diffuse = _LightColor0.rgb * lightIntensity * s.Albedo * steppedDiff;
